...
for s in $scenarios
        for c in $controllers
                ...
                sed  's/depth1_foraging/'$c'_foraging/g' exp/$s.argos > /tmp/exp-$c.argos
                sed "s/experiment length=\"0\"/experiment length=\"$timesteps\"/g" -i /tmp/exp-$c.argos
                sed "s/__current_date__/$c/g" -i /tmp/exp-$c.argos
