/**
 * @file energy_partitioning_controller.cpp
 *
 ...
 * Includes
 ******************************************************************************/
#include "fordyca/controller/depth1/energy_partitioning_controller.hpp"
#include <fstream>
 ...
 ******************************************************************************/
energy_partitioning_controller::energy_partitioning_controller(void)
    : ER_CLIENT_INIT("fordyca.controller.depth1.energy_partitioning"),
      m_cache_sel_matrix(),
      m_executive() {}

energy_partitioning_controller::~energy_partitioning_controller(void) = default;

/*******************************************************************************
 * Member Functions
 ******************************************************************************/
void energy_partitioning_controller::ControlStep(void) {
  ndc_pusht();
  if (nullptr != block()) {
...
void energy_partitioning_controller::Init(ticpp::Element& node) {
  /*
  ...
  executive()->task_abort_notify(
      std::bind(&energy_partitioning_controller::task_abort_cb,
                this,
                std::placeholders::_1));
...
void energy_partitioning_controller::non_unique_init(
    ticpp::Element& node,
    params::depth1::controller_repository* param_repo) {
...
void energy_partitioning_controller::task_abort_cb(const ta::polled_task*) {
  m_task_aborted = true;
} /* task_abort_cb() */

__rcsw_pure const ta::bi_tab* energy_partitioning_controller::active_tab(
    void) const {
  return m_executive->active_tab();
} /* active_tab() */

__rcsw_pure tasks::base_foraging_task* energy_partitioning_controller::current_task(
    void) {
  return dynamic_cast<tasks::base_foraging_task*>(
      m_executive.get()->current_task());
} /* current_task() */

__rcsw_pure const tasks::base_foraging_task* energy_partitioning_controller::
    current_task(void) const {
  return const_cast<energy_partitioning_controller*>(this)->current_task();
} /* current_task() */

void energy_partitioning_controller::executive(
...
TASK_WRAPPER_DEFINEC_PTR(transport_goal_type,
                         energy_partitioning_controller,
 ...
 ******************************************************************************/
int energy_partitioning_controller::current_task_depth(void) const {
  return executive()->graph()->vertex_depth(
      dynamic_cast<const ta::polled_task*>(current_task()));
} /* current_task_depth() */

int energy_partitioning_controller::current_task_id(void) const {
  return executive()->graph()->vertex_id(
      dynamic_cast<const ta::polled_task*>(current_task()));
} /* current_task_id() */

__rcsw_pure int energy_partitioning_controller::current_task_tab(void) const {
...
REGISTER_CONTROLLER(energy_partitioning_controller,
                    "energy_partitioning_controller");
