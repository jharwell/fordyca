...
void tasking_initializer::depth2_exec_est_init(
  ...
  if (task_params->exec_est.seed_enabled) {
    ...
    uint cs_min =
        task_params->exec_est.ranges.find("cache_starter")->second.get_min();
    uint cs_max =
        task_params->exec_est.ranges.find("cache_starter")->second.get_max();
    uint cf_min =
        task_params->exec_est.ranges.find("cache_finisher")->second.get_min();
    uint cf_max =
        task_params->exec_est.ranges.find("cache_finisher")->second.get_max();
    uint ct_min =
        task_params->exec_est.ranges.find("cache_transferer")->second.get_min();
    uint ct_max =
        task_params->exec_est.ranges.find("cache_transferer")->second.get_max();
    uint cc_min =
        task_params->exec_est.ranges.find("cache_collector")->second.get_min();
    uint cc_max =
        task_params->exec_est.ranges.find("cache_collector")->second.get_max();
