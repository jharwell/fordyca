...
void tasking_initializer::depth1_exec_est_init(
  ...
  if (task_params->exec_est.seed_enabled) {
    ...
    uint g_min =
        task_params->exec_est.ranges.find("generalist")->second.get_min();
    uint g_max =
        task_params->exec_est.ranges.find("generalist")->second.get_max();

    uint h_min =
        task_params->exec_est.ranges.find("harvester")->second.get_min();
    uint h_max =
        task_params->exec_est.ranges.find("harvester")->second.get_max();
    uint c_min =
        task_params->exec_est.ranges.find("collector")->second.get_min();
    uint c_max =
        task_params->exec_est.ranges.find("collector")->second.get_max();
    ER_INFO(
        ...
        c_min,
        c_max);
    static_cast<ta::polled_task*>(generalist)->exec_estimate_init(g_min, g_max);
