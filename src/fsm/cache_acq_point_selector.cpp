...
rmath::vector2d cache_acq_point_selector::operator()(
   ...
   */
  auto ul_center = rmath::vector2d(cache->real_loc().x() - cache->xsize() / 4.0 + m_arrival_tol,
                                   cache->real_loc().y() + cache->ysize() / 4.0 - m_arrival_tol);

  auto ur_center = rmath::vector2d(cache->real_loc().x() + cache->xsize() / 4.0 - m_arrival_tol,
                                   cache->real_loc().y() + cache->ysize() / 4.0 - m_arrival_tol);

  auto lr_center = rmath::vector2d(cache->real_loc().x() + cache->xsize() / 4.0 - m_arrival_tol,
                                   cache->real_loc().y() - cache->ysize() / 4.0 + m_arrival_tol);
  auto ll_center = rmath::vector2d(cache->real_loc().x() - cache->xsize() / 4.0 + m_arrival_tol,
                                   cache->real_loc().y() - cache->ysize() / 4.0 + m_arrival_tol);

  /* find closest center */
  std::list<rmath::vector2d> centers = {ul_center,
                                        ur_center,
                                        lr_center,
                                        ll_center};
  auto closest = *std::min_element(centers.begin(),
                                   centers.end(),
                                   [&](const auto& c1, const auto& c2) {
                                     return (c1 - robot_loc).length() <
                                     (c2 - robot_loc).length();
                                   });
  ...
  ER_ASSERT(cache->contains_point(loc), "Cache%d@%s/%s does not contain %s",
            cache->id(),
