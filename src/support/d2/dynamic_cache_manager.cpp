...
dynamic_cache_manager::create(const cache_create_ro_params& c_params,
                              const cds::block3D_vectorno& c_all_blocks) {
  if (auto for_creation = creation_blocks_alloc(
          c_params.current_caches, c_params.clusters, c_all_blocks)) {
...
bool dynamic_cache_manager::creation_blocks_alloc_check(
  ...
  if (c_blocks.usable.size() < mc_cache_config.dynamic.min_blocks) {
    ...
    uint count = 0;
    std::for_each(
        c_blocks.usable.begin(), c_blocks.usable.end(), [&](const auto& b) {
          count +=
              (b->is_out_of_sight() ||
