 ...
 * Includes
 ******************************************************************************/
#include "fordyca/diagnostics/depth0/robot_stat_collector.hpp"
...
void robot_stat_collector::collect(const collectible_diagnostics& diagnostics) {
  m_stats.n_exploring_for_block += diagnostics.is_exploring_for_block();
  m_stats.n_avoiding_collision += diagnostics.is_avoiding_collision();
  m_stats.n_transporting_to_nest += diagnostics.is_transporting_to_nest();

  m_stats.n_acquiring_block += diagnostics.is_acquiring_block();
  m_stats.n_vectoring_to_block += diagnostics.is_vectoring_to_block();
} /* collect() */
