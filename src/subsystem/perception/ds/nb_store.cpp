 ...
 ******************************************************************************/
nb_store::nb_store(const cspconfig::ntimestep_config* const config)
    : ER_CLIENT_INIT("fordyca.subsystem.perception.ds.nb_store"),
      mc_N_timesteps(config->N) {}

/*******************************************************************************
 * Member Functions
 ******************************************************************************/
model_update_result
nb_store::cache_update(tracked_cache_type&& cache) {
...
bool nb_store::cache_remove(carepr::base_cache* const victim) {
...
model_update_result
nb_store::block_update(tracked_block_type&& block_in) {
...
bool nb_store::block_remove(crepr::base_block3D* const victim) {
  auto range = tracked_blocks().values_range();
