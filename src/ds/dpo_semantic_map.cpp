 ...
 ******************************************************************************/
dpo_semantic_map::dpo_semantic_map(const cspconfig::perception_config* c_config,
...
model_update_result dpo_semantic_map::block_update(tracked_block_type&& block) {
   ...
   * FORDYCA#229.
   */
  if (ds::model_update_status::ekBLOCK_MOVED == res.reason ||
      ds::model_update_status::ekNEW_BLOCK_ADDED == res.reason) {
...
model_update_result dpo_semantic_map::cache_update(tracked_cache_type&& cache) {
  ...
  if (ds::model_update_status::ekCACHE_UPDATED == res.reason ||
      ds::model_update_status::ekNEW_CACHE_ADDED == res.reason) {
