...
bool creation_verifier::sanity_checks(
    const cads::acache_vectorro& c_caches,
...
bool creation_verifier::sanity_check_internal_consistency(
...
bool creation_verifier::sanity_check_free_block_overlap(
...
bool creation_verifier::sanity_check_block_cluster_overlap(
    const carepr::arena_cache* cache,
...
bool creation_verifier::sanity_check_nest_overlap(
    const carepr::arena_cache* cache,
...
bool creation_verifier::sanity_check_cross_consistency(
...
bool creation_verifier::sanity_check_cache_overlap(
    const cads::acache_vectorro& caches) const {
  for (const auto& c1 : caches) {
    auto c1_xspan = c1->xrspan();
    auto c1_yspan = c1->yrspan();
