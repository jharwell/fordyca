...
#ifndef INCLUDE_FORDYCA_CONFIG_ORACLE_ENTITIES_ORACLE_PARSER_HPP_
#define INCLUDE_FORDYCA_CONFIG_ORACLE_ENTITIES_ORACLE_PARSER_HPP_
...
class entities_oracle_parser : public rconfig::xml::xml_config_parser {
 public:
  explicit entities_oracle_parser(uint level) : xml_config_parser(level) {}
  ...
  std::shared_ptr<entities_oracle_config> config_get(void) const {
    return m_config;
  }
 ...
 private:
  ...
  /* clang-format off */
  std::shared_ptr<entities_oracle_config> m_config{nullptr};
  /* clang-format on */
};

NS_END(oracle, config, fordyca);

#endif /* INCLUDE_FORDYCA_CONFIG_ORACLE_ENTITIES_ORACLE_PARSER_HPP_ */
