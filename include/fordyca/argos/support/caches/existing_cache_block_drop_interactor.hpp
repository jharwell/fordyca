...
#ifndef INCLUDE_FORDYCA_ARGOS_SUPPORT_CACHES_EXISTING_CACHE_BLOCK_DROP_INTERACTOR_HPP_
#define INCLUDE_FORDYCA_ARGOS_SUPPORT_CACHES_EXISTING_CACHE_BLOCK_DROP_INTERACTOR_HPP_
...
class existing_cache_block_drop_interactor
 ...
 public:
  ...
  existing_cache_block_drop_interactor(carena::caching_arena_map* const map_in,
                                       fastv::env_dynamics* envd)
      : ER_CLIENT_INIT("fordyca.argos.support.caches.existing_cache_block_drop_interactor"),
        m_map(map_in),
        m_penalty_handler(
            envd->penalty_handler(fastv::cache_op_src::ekEXISTING_CACHE_DROP)) {}
  ...
  void operator()(TController& controller, rtypes::timestep t) {
    if (m_penalty_handler->is_serving_penalty(controller)) {
      ...
      m_penalty_handler->penalty_init(
          controller, fastv::cache_op_src::ekEXISTING_CACHE_DROP, t);
    }
  }
 ...
 private:
  ...
  void execute_cache_block_drop(TController& controller,
    ...
    controller.block_manip_recorder()->record(fmblocks::block_manip_events::ekCACHE_DROP,
                                              penalty.penalty());
    /*
  ...
  fastv::cache_op_penalty_handler*const m_penalty_handler;
  /* clang-format on */
};

NS_END(caches, support, argos, fordyca);

#endif /* INCLUDE_FORDYCA_ARGOS_SUPPORT_CACHES_EXISTING_CACHE_BLOCK_DROP_INTERACTOR_HPP_ */
